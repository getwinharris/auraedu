---
version: alpha
name: AuraEdu (bapXaura-inspired)
...
components:
  ...
  hero-band-dark:
    background: linear-gradient(135deg, {colors.hero-dark-from} 0%, {colors.hero-dark-to} 100%)
    textColor: "{colors.on-dark}"
    padding: "{spacing.hero}"
  footer-region:
    backgroundColor: "{colors.canvas}"
...
- Signature bapXaura mint green (`{colors.brand-green}`) reserved for accent CTAs, active states, and feature confirmations
...
### Brand & Accent
- **bapXaura Mint** (`{colors.brand-green}`): Signature accent — used on hero "Get started" pill button, green checkmarks in feature lists, featured pricing tier border accent, sidebar active indicator dots.
...
| `{typography.heading-2}` | 36px | 600 | 1.20 | -0.5px | Section headlines ("Apply to the bapXaura startup program") |
...
**`testimonial-card-feature`** — Bright orange large testimonial card with photo + quote ("Cursor — Every YC batch we consistently see the top performing startups use bapXaura to build their docs.").
...
- Left: bapXaura wordmark + horizontal link list (Solutions, Pricing, Customers, Documentation, Changelog).
...
**`logo-wall-item`** — Customer logo cell in 6-up trust-row grids ("Anthropic / Cognition / bapXaura / Vercel / react / Lovable", "Stripe / Block / PayPal / Compound / Auth").
...
### Do
- Reserve `{colors.brand-green}` (bapXaura mint) for accent CTAs and active state indicators only — even one accent button per viewport carries weight
