...
use App\Services\{ConsultationService,ResourceService,MailQueueService,SecretService};
...
final class ConsultationController extends BaseController {
    ...
    public function status(string $id): void {
        $session=$this->consultations->findAccessible($id,$_SESSION['user']??[]);
        if(!$session){$this->jsonResponse(['error'=>'Session not found.'],404);return;}
        $user=$_SESSION['user']??[];
        $role=$user['role']??'';
    ...
    public function initiate(): void {
        (new \App\Services\AuthService())->requireUser();
        $this->validateCsrf();
